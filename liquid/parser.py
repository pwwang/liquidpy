...
class LiquidLine:
    ...
    def __repr__(self):
        ...
        return '<LiquidLine {!r} (compiled from #{!r})>'.format(
            self.line,
            self.lineno if not self.parser else '{}:L{}'.format(
...
class LiquidParser: # pylint: disable=too-many-instance-attributes
    ...
    def parse(self, first=True):
        ...
        if (self.extends and
                tag and
                tag not in (LIQUID_STATE_OPENTAG,
                            LIQUID_STATE_OPENTAG_COMPACT)):
    ...
    def _expect_end_state(self, tag, name, rest=''):
        """
        ...
        endstates = []
        for opentag, closetag in zip(opentags * 2, sorted(closetags * 2)):
            endstates.extend(
                '%s%send%s%s%s' % (opentag,
                                   openspace,
                                   name,
                                   closespace,
                                   closetag)
                for openspace, closespace in zip(['', ' ', '', ' '],
                                                 ['', '', ' ', ' '])
            )
        ...
        if self.endtag in LIQUID_COMPACT_TAGS or self.mode == 'compact':
            ...
            string = string2
        if (endstate.startswith(LIQUID_STATE_OPENTAG_COMPACT) or
                self.mode == 'compact'):
            string = string.rstrip(' \t').rstrip('\n')
        self.endtag = LIQUID_STATE_CLOSETAG_COMPACT \
            if endstate.endswith(LIQUID_STATE_CLOSETAG_COMPACT) \
            else LIQUID_STATE_CLOSETAG
    ...
    def parse_literal(self, string, tag):
        ...
        if not string:
            return
        LOGGER.debug(
            "- Literals wrapped by tags (%s, %s) found at line %s: %r",
            self.endtag,
            tag,
            self.lineno,
            _shorten(string)
    ...
    def parse_statement(self, tag):
        ...
        if name not in self.nodes:
            self.raise_ex(
                'Unknown statement: {!r}'.format('end' + name if end else name)
            )
        if end:
            if rest:
    ...
    def parse_block(self, tag, name):
        ...
        block, nlines, leading_lines = self._expect_end_state(tag,
                                                              'block',
                                                              name)
        ...
        if self.extends:
            ...
            self.blocks[name] = LiquidParser(LiquidStream.from_string(block),
                                             ...
                                             self.filename,
                                             self.prev,
                                             self.lineno + leading_lines)
            self.blocks[name].mode = self.mode
            self.lineno += nlines
        # I am the parent
        elif self.prev and name in self.prev[1].blocks:
            LOGGER.debug("START parsing block %r from file: %s",
                         name,
                         self.prev[1].filename)
            self.prev[1].blocks[name].parse()
        else:
            LOGGER.debug(
                "- Didn't find any blocks to replace, parsing block: %s",
                name
            )
    ...
    def parse_extends(self, path):
        ...
        for hist in self.history:
            if hist != 'mode':
                self.raise_ex(
                    "Statement 'extends' should be placed "
                    "at the top or after 'mode'"
                )
        ...
        self.extends = LiquidParser(None,
                                    self.precode,
                                    self.code,
                                    extpath,
                                    prev=(self.lineno, self))
