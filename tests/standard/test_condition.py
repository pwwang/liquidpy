...
def render(left, op, right):
    left = TagFragConst(left)
    right = TagFragConst(right)
    return TagFragOpComparison(left, op, right).render({}, {})
...
def op_compare():
    def func(left, op, right):
        left = TagFragConst(left) if not isinstance(left, TagFrag) else left
        right = TagFragConst(right) if not isinstance(right, TagFrag) else right
...
def test_invalid_comparation_operator(render_func):
...
def test_or_condition(op_compare):
    opc = op_compare(1, '==', 2)
    assert opc.render({}, {}) == False
    opc = op_compare(opc, 'or', op_compare(2, '==', 1))
    assert opc.render({}, {}) == False
    opc = op_compare(opc, 'or', op_compare(1, '==', 1))
    assert opc.render({}, {}) == True

def test_and_condition(op_compare):
    opc = op_compare(1, '==', 1)
    assert opc.render({}, {}) == True
    opc = op_compare(opc, 'and', op_compare(2, '==', 2))
    assert opc.render({}, {}) == True
    opc = op_compare(opc, 'and', op_compare(2, '==', 1))
    assert opc.render({}, {}) == False
...
def test_left_or_right_may_contain_operators(op_compare):
    ...
    opc = op_compare(TagFragVar('one'), '==', TagFragVar('another'))
